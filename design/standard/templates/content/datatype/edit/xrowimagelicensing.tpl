...
{default attribute_base=ContentObjectAttribute}
    ...
    <div class="block">
        <div class="selectlist">
            <div class="element">
                <label><input {if $attribute.data_text|compare("0")}checked="checked"{/if} type = "radio" value = "0" id="{$id_base}_selectlist" class="ezcc-{$attribute.object.content_class.identifier} ezcca-{$attribute.object.content_class.identifier}_{$attribute.contentclass_attribute_identifier}" name="{$attribute_base}_bildlizenz_selected_{$attribute.id}">{'Delete on'|i18n( 'extension/xrowimagelicensing/datatypes' )}</label>
                <label><input {if $attribute.data_text|compare("1")}checked="checked"{/if} type = "radio" value = "1" id="{$id_base}_selectlist" class="ezcc-{$attribute.object.content_class.identifier} ezcca-{$attribute.object.content_class.identifier}_{$attribute.contentclass_attribute_identifier}" name="{$attribute_base}_bildlizenz_selected_{$attribute.id}">{'No expiration date'|i18n( 'extension/xrowimagelicensing/datatypes' )}</label>
            </div>
        </div>
        <div class="date">
            <div class="element">
